...
UniValue generate(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3) {
        throw std::runtime_error(
            ...
            "3. extra_multiply (Variable Block Reward). Specify as power of 2 to acheive best possible reward. Defaults to regular bitcoin block reward if not specified.\n"
            "\nResult:\n"
    ...
    if (!request.params[2].isNull()) {
        extra_multiply = request.params[2].get_int();
    }    
    ...
    return generateBlocks(coinbase_script, num_generate, max_tries, extra_multiply, true);
}
